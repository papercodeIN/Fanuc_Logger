...
all_column_name_df = pd.read_csv("Init_Data/column_name.csv")
all_column_name = all_column_name_df.Variable.to_list()
#print(all_column_name)
#M_DF = pd.DataFrame(columns = all_column_name)

display_column_name_df = pd.read_csv("Init_Data/display_column_name.csv")
display_column_name = display_column_name_df.Variable.to_list()
#print(display_column_name)
        ...
        try:
            while 1 :
                ...
                if  (bit_status(int(conn.readpmc(1,1,0,1)[0]), 6) == 0) and (data_logged == 0) and (int(conn.readpmc(1,5,32,1)[32]) == 1) :
                    ...
                    try:
                        M_DF = M_DF.append(pd.DataFrame([[ v for k, v in conn.readmacro(500,594).items()]], columns=all_column_name)[display_column_name],ignore_index=True)
                        M_DF.to_csv(f"Report/Report_{date.today()}.csv", mode='a', header=not os.path.exists(f"Report_{date.today()}.csv"))
