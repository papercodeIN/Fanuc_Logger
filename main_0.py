...
column_name_df = pd.read_csv("Init_Data/column_name.csv")
all_column_name = column_name_df.Variable.to_list()
        ...
        try:
            while 1 :
                ...
                if  (bit_status(int(conn.readpmc(1,1,0,1)[0]), 6) == 0) and (data_logged == 0) and (int(conn.readpmc(1,5,32,1)[32]) == 1) :
                    ...
                    try:
                        M_DF = M_DF.append(pd.DataFrame([[ v for k, v in conn.readmacro(500,594).items()]], columns=all_column_name),ignore_index=True)
